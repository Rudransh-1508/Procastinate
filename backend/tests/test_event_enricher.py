...
def _task_and_open_event(db):
    ...
    eid = generate_id()
    db.execute(
        """INSERT INTO procrastination_events (id, task_id, detected_at, delay_start_at, delay_hours)
           VALUES (?, 't1', ?, ?, 6.0)""",
        (eid, "2026-06-01T10:00:00+00:00", "2026-06-01T08:00:00+00:00"),
    )
    db.commit()
    return eid
...
def test_enricher_noop_when_aw_unavailable(fresh_db, monkeypatch):
    ...
    assert EventEnricher().enrich_pending_event(eid) is None
...
def test_enricher_writes_displacement(fresh_db, monkeypatch):
    ...
    out = EventEnricher().enrich_pending_event(eid)
...
def test_refresh_profile_state(fresh_db):
    ...
    res = InsightGenerator().refresh_profile_state()
    assert res["total_events"] == 1
    assert res["confidence"] == "low"

    row = db.execute("SELECT * FROM profile_state WHERE id = 1").fetchone()
